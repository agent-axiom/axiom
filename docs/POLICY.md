...
It blocks known-dangerous direct commands and shell control operators, but it cannot prevent a trusted interpreter command from doing arbitrary work. Treat AXIOM policy as workflow enforcement and audit evidence, not OS isolation.

## Command Policy Profiles

Verification commands run through the tool broker with policy checks, timeouts, and stdout/stderr capture limits.
...
`.axiom/policy.yaml` is privileged trust configuration. It should be owned by a human or platform operator, not opportunistically modified by the task agent. If a task changes `.axiom/policy.yaml` or `.axiom/policy.yml`, deterministic review requests changes before finish even when the file is inside the declared plan scope.
...
Command adapters are trusted local commands. AXIOM does not sandbox them. They run in the task worktree and can read or write files available to the current OS user.
